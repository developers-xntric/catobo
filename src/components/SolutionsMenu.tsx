...
export const engineeringSolutionLinks: SolutionLink[] = [
  ["Cable Containment Systems", "/engineering/cable-trunking", 3970],
  ["Lightning Protection Systems", "/engineering/lightening-protection-systems", 3972],
  ["Industrial Products", "/engineering/explosion-proof-systems", 3973],
].map(([label, href, image]) => ({ label: String(label), href: String(href), image: Number(image) }));

export default function SolutionsMenu({ onClose, contactHref, links }: { onClose: () => void; contactHref: string; links: SolutionLink[] }) {
  const [preview, setPreview] = useState<SolutionLink>(links[0]);
    ...
    <div className="grid grid-cols-[0.95fr_1.9fr_0.95fr] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-2xl">
      ...
      <div className="flex min-w-0 flex-col gap-4 p-6">
        ...
        <div className="grid flex-1 grid-flow-col grid-rows-6 grid-cols-2 gap-x-5 gap-y-1">
          {links.map(item => <Link key={item.href} href={item.href} onClick={onClose} onMouseEnter={() => setPreview(item)} onFocus={() => setPreview(item)} className="group relative flex min-h-10 items-center rounded px-1 py-2 pr-7 text-xs leading-relaxed transition-colors hover:bg-sky-50 hover:text-[#168dca] focus-visible:outline-2 focus-visible:outline-sky-600">
            {item.label}<ArrowRight aria-hidden="true" className="absolute right-1 top-2 size-3.5 text-[#0f2453] transition-colors group-hover:text-[#168dca] group-focus-visible:text-[#168dca]" />
          </Link>)}
        </div>
