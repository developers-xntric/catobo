...
export function ProcessTimeline({ title, columns = 4, steps, subtitle }: ProcessTimelineProps) {
    return (
            ...
            >
                {steps.map((step) => (
                    <div key={step.number} className="flex flex-col items-start gap-3">
                        ...
                        {step.description && (
                            <p className="text-[12.5px] text-[#636363] leading-snug">{step.description}</p>
                        )}
                    </div>
                ))}
            </div>
